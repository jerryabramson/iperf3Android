    ...
    Scaffold(
        topBar = {
            TopAppBar(
                ...
                actions = { RunButton(viewModel, uiState.isRunning) },
        ...
        Column(modifier = Modifier.padding(padding).fillMaxSize()) {
            ...
            NumericInputs(uiState, viewModel, fieldColors, monoStyle)
            Text("FOOOBAR1")

            //SettingsSection(uiState, viewModel, monoStyle)
            //RunButton(viewModel, uiState.isRunning)
            Text("FOOOBAR2")
            ResultsSection(uiState, monoStyle)
            Text("FOOOBAR3")
            RunningSection(uiState, monoStyle)
            Text("FOOOBAR4")
...
private fun HostInputRow(
    ...
    Row(
        ...
        SettingsSection(uiState, viewModel, monoStyle = monoStyle.copy(fontSize = 13.sp))
        //RunButton(viewModel, uiState.isRunning)
    }
}

@Composable
private fun RunButton(viewModel: Iperf3RunViewModel, isRunning: Boolean) {
...
private fun ResultsSection(
    ...
    if (uiState.isRunning || uiState.isFinished.not()) return
    ...
    HorizontalDivider(modifier = Modifier.fillMaxWidth(), thickness = 2.dp, color = MaterialTheme.colorScheme.tertiary)
...
private fun IperfMessagesSection(
    ...
    val shouldShow = (uiState.isVerbose || uiState.isDebugging || uiState.iperf3Messages.size <= 1) &&
        (uiState.isRunning || uiState.isFinished) &&
        uiState.iperf3Messages.isNotEmpty()
    ...
    Row() {
        ...
        if (!uiState.isFinished && uiState.latestLine.isEmpty() && !uiState.isDebugging) {
            LoadingFooter()
